...
*/
class EmploymentContractTemplateController extends Controller{

    /**
     * @Route("/", name="employment_contract_default")
     */
    public function indexAction(){
        return $this->redirect($this->generateUrl('employment_contract_list', ['page' => 1]));
    }
    /**
    ...
    public function listAction($id = 0, $page = 0, $uid = 0){
        ...
        if($id > 0){
            ...
            $total = $this->getDoctrine()->getRepository('KatropineAdminBundle:EmploymentContractTemplate')->countAllByCompanyId($id);
            $route_name = 'company_employment_contracts';
        }else{
            $total = $this->getDoctrine()->getRepository('KatropineAdminBundle:EmploymentContractTemplate')->countAll($q);
    ...
    public function saveAction(Request $request, $id = 0, $cid = 0){
        ...
        $returnUrl = $this->generateUrl('employment_contract_list');
         if($id > 0){
            $contract = $this->getDoctrine()->getEntityManager()->find("KatropineAdminBundle:EmploymentContractTemplate", $id);
        }else{
            $contract = new EmploymentContractTemplate();
        }
        if($cid > 0){
